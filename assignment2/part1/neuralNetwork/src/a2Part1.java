...
public class a2Part1 {
    ...
    public static void main(String[] _ignored) {
        ...
        double[][][] firstEpoch = nn.backward_propagate_error(instances[0], forwardPassOutputs[0], forwardPassOutputs[1], integer_encoded[0]);
        nn.update_weights(firstEpoch[0],firstEpoch[1]);
        ...
        int[] testPredict = nn.predict(instances_test);
        double acc = Double.NaN;
        double correct = 0.0;
        label_encoder = new LabelEncoder(labels_test);
        int[] desiredOutputs = label_encoder.intEncode(labels_test);
        for(int i = 0; i < testPredict.length; i++){
            if (testPredict[i] == desiredOutputs[i]) correct++;
        }
        acc = (correct / instances_test.length)*100;
        System.out.println("For the test set:");
        System.out.println(correct+" out of "+instances_test.length+ " correct results.");
        System.out.println("Accuracy for test set = " + acc + "%");
